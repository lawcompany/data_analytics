...
# 각 토큰화의 시간을 보면 나노세컨드 -> 마이크로 세컨드로 시간이 증가한 것을 볼수있습니다.
%timeit ", ".join(tokenizer.tokenize(samples))
%timeit ", ".join(tokenizer_add.tokenize(samples))

# 각 결과를 보면 통매음, 도촬 등의 핵심단어들이 토큰화되는 것을 알 수 있습니다.
", ".join(tokenizer.tokenize(samples))
", ".join(tokenizer_add.tokenize(samples))
...
'''
아래의 코드는 토큰화의 시간이 
tokenzier의 add_tokens(list(tokens.token.values)) 함수를 쓰면 토큰화의 시간이 오래걸리는 현상이 발생하는데(이렇게 되면 token의 종류가 다르게 들어가서.. 어찌저찌 됬던것 같은데 가물가물...)
이 현상을 해결하기 위하여 토큰나이저를 save한뒤 json 파일을 직접 수정하여 tokenizer의 성능을 향상시키기 위한 코드입니다.
빅데이터라서 너무 오래걸리거나 모델 서빙처럼 리드타임이 중요하지 않으면 굳이 사용하시지 않아도 됩니다.

위에처럼 토큰화를 하면 모델 성능에 어떻게 영향을 미치는지 
'''
